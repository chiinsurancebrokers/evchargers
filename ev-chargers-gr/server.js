...
// Βέλτιστο σημείο φόρτισης — ΧΙΛΙΟΜΕΤΡΙΚΟ κριτήριο.
// ΣΗΜΑΝΤΙΚΟ: τα νούμερα (θέσεις/SoC) υπολογίζονται ΝΤΕΤΕΡΜΙΝΙΣΤΙΚΑ από τον client
// και στέλνονται έτοιμα. Εδώ το AI κάνει ΜΟΝΟ εξήγηση (Claude) και έλεγχο (OpenAI),
// χωρίς να ξαναϋπολογίζει αριθμητική (αποφυγή σφαλμάτων LLM).
app.post('/api/charge-plan', async (req, res) => {
  ...
  try {
    ...
    const b = t.battery || {};
    const p = t.plan || {};
    const stopsTxt = (p.stops || []).length
      ? p.stops.map((s, i) => `  ${i + 1}. ${s.name} — ${s.power || '?'} kW, στα ${s.alongKm} km, άφιξη ~${s.arriveSoc}% → φόρτιση έως ${s.leaveSoc ?? b.chargeToSoc}%`).join('\n')
      : `  (καμία στάση — άφιξη στον προορισμό ~${p.arriveSoc}%)`;
    const ctx =
`Διαδρομή: ${t.from} → ${t.to}, ${t.routeKm} km. Αυτονομία 100→0% ≈ ${b.rangeFull} km (κατανάλωση ${b.consEff} kWh/100km).
Εκκίνηση ${b.startSoc}% · εφεδρεία/όριο ${b.reserveSoc}% · φόρτιση έως ${b.chargeToSoc}%.
ΥΠΟΛΟΓΙΣΜΕΝΟ ΣΧΕΔΙΟ (από μηχανή — τα νούμερα είναι σωστά, ΜΗΝ τα ξαναϋπολογίσεις):
${stopsTxt}
Άφιξη στον προορισμό: ~${p.arriveSoc}%.`;

    // Στάδιο 1: εξήγηση από Claude (ή τον πρώτο διαθέσιμο)
    const explainProvider = aiProviders().includes('anthropic') ? 'anthropic' : defaultProvider();
    let explanation = null;
    try {
      const ex = await callAI(explainProvider,
`${ctx}

Εξήγησε ΣΥΝΤΟΜΑ (≤90 λέξεις, ελληνικά) γιατί αυτό το σχέδιο είναι βέλτιστο με ΧΙΛΙΟΜΕΤΡΙΚΟ κριτήριο (φόρτιση όσο πιο αργά γίνεται με ασφάλεια, μέγιστη απόσταση ανά στάση, σεβασμός εφεδρείας). Χρησιμοποίησε ΜΟΝΟ τα παραπάνω νούμερα. Μην προτείνεις τιμές.`, 400);
      explanation = { text: ex.text, model: ex.model, provider: ex.provider };
    } catch (e) { explanation = { error: e.message, provider: explainProvider }; }

    // Στάδιο 2: έλεγχος από OpenAI (αν υπάρχει & διαφορετικό)
    let review = null;
    if (aiProviders().includes('openai') && explainProvider !== 'openai') {
      try {
        const rv = await callAI('openai',
`${ctx}

Είσαι ΑΝΕΞΑΡΤΗΤΟΣ ελεγκτής. Τα νούμερα είναι ήδη υπολογισμένα από μηχανή — ΜΗΝ τα ξαναϋπολογίσεις. Έλεγξε μόνο τη ΛΟΓΙΚΗ: (1) καμία στάση/άφιξη κάτω από την εφεδρεία ${b.reserveSoc}%; (2) φορτίζει όσο πιο αργά γίνεται (μέγιστη απόσταση ανά σκέλος); (3) ελάχιστες στάσεις. Ξεκίνα με «✅ Συμφωνώ» ή «⚠️ Προσοχή» + 1–2 σημεία. ≤80 λέξεις, ελληνικά.`, 400);
        review = { text: rv.text, model: rv.model, provider: 'openai' };
      } catch (e) { review = { error: e.message, provider: 'openai' }; }
    } else {
      // Ο επαληθευτής OpenAI δεν είναι διαθέσιμος — δήλωσέ το ρητά στο UI
      review = { unavailable: true, reason: aiProviders().includes('openai') ? 'same-provider' : 'no-key' };
    }

    res.json({ explanation, review });
  } catch (err) {
    res.status(500).json({ error: `Αποτυχία: ${err.message}` });
  }
});
