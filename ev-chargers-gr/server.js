...
function aiProviders() {
  const list = [];
  if (ANTHROPIC_API_KEY) list.push('anthropic');
  if (OPENAI_API_KEY) list.push('openai');
  return list;
}
function defaultProvider() { return aiProviders()[0] || null; }
...
async function callOpenAI(prompt, maxTokens) {
  const r = await fetch(`${OPENAI_BASE}/v1/chat/completions`, {
    ...
    body: JSON.stringify({ model: OPENAI_MODEL, max_completion_tokens: maxTokens, messages: [{ role: 'user', content: prompt }] }),
  });
...
// Δρομολόγηση σε πάροχο. Αν ο ζητούμενος δεν είναι διαθέσιμος, πέφτει στον προεπιλεγμένο.
async function callAI(provider, prompt, maxTokens) {
  const p = provider && aiProviders().includes(provider) ? provider : defaultProvider();
  if (!p) throw new Error('Κανένας πάροχος AI');
  return p === 'openai' ? callOpenAI(prompt, maxTokens) : callAnthropic(prompt, maxTokens);
}
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    dynamicUpdatedAt: cache.dynamicUpdatedAt,
    error: cache.error,
    aiEnabled: aiProviders().length > 0,
...
// Δεύτερη γνώμη: στέλνει το πλάνο σε έναν ή ΚΑΙ στους δύο παρόχους (Claude + OpenAI)
// και επιστρέφει τις απόψεις. Τα κλειδιά μένουν ΜΟΝΟ στον server.
app.post('/api/second-opinion', async (req, res) => {
  const provs = aiProviders();
  if (!provs.length) {
    return res.status(503).json({
      error: 'Η δεύτερη γνώμη είναι απενεργοποιημένη. Όρισε ANTHROPIC_API_KEY ή/και OPENAI_API_KEY στο Railway.',
    });
  }
  try {
    const t = req.body || {};
...
Δώσε: (1) μια γρήγορη κρίση αν το πλάνο είναι ασφαλές/ρεαλιστικό, (2) τυχόν ρίσκα (π.χ. χαμηλό % άφιξης, αργοί φορτιστές, μεγάλο κενό), (3) μία εναλλακτική στρατηγική στάσεων αν υπάρχει καλύτερη. Μέγιστο ~150 λέξεις, καθαρά και πρακτικά, χωρίς εισαγωγικά κλισέ.`;
    ...
    // Επιλογή παρόχων: 'both' ή κενό+δύο κλειδιά -> και οι δύο· αλλιώς ο ζητούμενος/προεπιλεγμένος.
    let targets;
    if (t.provider === 'both') targets = provs;
    else if (t.provider && provs.includes(t.provider)) targets = [t.provider];
    else targets = provs; // default: όσοι πάροχοι υπάρχουν

    const settled = await Promise.allSettled(targets.map((p) => callAI(p, userPrompt, 700)));
    const opinions = settled.map((r, i) =>
      r.status === 'fulfilled'
        ? { provider: targets[i], model: r.value.model, text: r.value.text }
        : { provider: targets[i], error: r.reason.message });
    res.json({ opinions });
  } catch (err) {
...
app.post('/api/route-pois', async (req, res) => {
  if (!aiProviders().length) {
    return res.status(503).json({ error: 'Η λειτουργία είναι ανενεργή. Όρισε ANTHROPIC_API_KEY ή OPENAI_API_KEY στο Railway.' });
  }
  try {
    const t = req.body || {};
...
Κανόνες: 3–5 items ανά κατηγορία· "area" = πόλη/περιοχή κοντά στη διαδρομή· "note" σύντομο (≤14 λέξεις), στα ελληνικά· μόνο πραγματικά, γνωστά μέρη.`;
    ...
    let result;
    try { result = await callAI(defaultProvider(), prompt, 1600); }
    catch (e) { return res.status(502).json({ error: e.message }); }
    let text = (result.text || '').replace(/^```json\s*/i, '').replace(/^```\s*/, '').replace(/```$/, '').trim();
    let parsed = null;
    try { parsed = JSON.parse(text); } catch (e) { /* fallback ως raw */ }
    res.json({ categories: parsed && parsed.categories ? parsed.categories : null, raw: parsed ? null : text, model: result.model });
  } catch (err) {
    res.status(500).json({ error: `Αποτυχία: ${err.message}` });
  }
});

// Βέλτιστο σημείο φόρτισης — με ΧΙΛΙΟΜΕΤΡΙΚΟ κριτήριο (όχι τιμή). Το Claude επιλέγει
// και αιτιολογεί τη στάση/στάσεις, με βάση τα προϋπολογισμένα δεδομένα που στέλνουμε.
app.post('/api/charge-plan', async (req, res) => {
  if (!aiProviders().length) {
    return res.status(503).json({ error: 'Η λειτουργία είναι ανενεργή. Όρισε ANTHROPIC_API_KEY ή OPENAI_API_KEY στο Railway.' });
  }
  try {
    const t = req.body || {};
    const b = t.battery || {};
    const cand = (t.candidates || []).slice(0, 60);
    const lines = cand.length
      ? cand.map((c, i) => `- [${i}] ${c.name} | ${c.power}kW | ${c.alongKm} km | άφιξη ${c.arriveSoc}% | ${c.detour}km εκτός | ${c.status}`).join('\n')
      : '(κανένας φορτιστής εντός κριτηρίων κατά μήκος της διαδρομής)';

    const prompt =
`Σχεδιασμός φόρτισης ηλεκτρικού οχήματος. ΚΡΙΤΗΡΙΟ: ΧΙΛΙΟΜΕΤΡΙΚΟ (μέγιστη απόσταση ανά φόρτιση) — ΑΓΝΟΗΣΕ τιμές/χρεώσεις.

Διαδρομή: ${t.from} → ${t.to}, συνολικά ${t.routeKm} km.
Μπαταρία: ${b.capacity} kWh · κατανάλωση ${b.consEff} kWh/100km · αυτονομία 100→0% ≈ ${b.rangeFull} km.
Εκκίνηση ${b.startSoc}% · εφεδρεία/όριο ${b.reserveSoc}% · φόρτιση έως ${b.chargeToSoc}%.

Υποψήφιοι φορτιστές (km = θέση στη διαδρομή· "άφιξη %" = εκτιμώμενο SoC αν οδηγήσεις ώς εκεί ΧΩΡΙΣ ενδιάμεση φόρτιση από την αφετηρία):
${lines}

Βρες το/τα ΒΕΛΤΙΣΤΟ/-Α σημείο/-α φόρτισης ώστε:
1) να μη πέφτεις ποτέ κάτω από την εφεδρεία ${b.reserveSoc}%,
2) να φορτίζεις όσο πιο αργά γίνεται (να διανύεις τη μέγιστη δυνατή απόσταση πριν κάθε στάση),
3) να ελαχιστοποιείς τον αριθμό στάσεων,
4) αν ο προορισμός είναι πιο μακριά από μία φόρτιση, άλυσε στάσεις: σε κάθε στάση φόρτισε έως ${b.chargeToSoc}% και ξαναϋπολόγισε από εκεί (τα "άφιξη %" των επόμενων υποψηφίων αναφέρονται στην αφετηρία, διόρθωσέ τα νοερά μετά από φόρτιση),
5) επί ισοπαλίας θέσης, προτίμησε μεγαλύτερη ισχύ (kW).

Επέστρεψε ΜΟΝΟ έγκυρο JSON (χωρίς markdown):
{"needsCharge":true|false,"arriveSoc":<αριθμός: εκτιμώμενο % στον προορισμό>,"stops":[{"name":"","alongKm":0,"power":0,"arriveSoc":0,"leaveSoc":${b.chargeToSoc},"reason":"≤15 λέξεις"}],"summary":"≤25 λέξεις"}`;

    // Στάδιο 1: το Claude (αν υπάρχει) προτείνει το βέλτιστο σημείο
    const planProvider = aiProviders().includes('anthropic') ? 'anthropic' : defaultProvider();
    let result;
    try { result = await callAI(planProvider, prompt, 900); }
    catch (e) { return res.status(502).json({ error: e.message }); }
    let text = (result.text || '').replace(/^```json\s*/i, '').replace(/^```\s*/, '').replace(/```$/, '').trim();
    let parsed = null;
    try { parsed = JSON.parse(text); } catch (e) { /* fallback */ }

    // Στάδιο 2: το OpenAI (αν υπάρχει & είναι διαφορετικό) ΕΛΕΓΧΕΙ την πρόταση
    let review = null;
    if (parsed && aiProviders().includes('openai') && planProvider !== 'openai') {
      const stopsTxt = (parsed.stops || []).length
        ? parsed.stops.map((s, i) => `  ${i + 1}. ${s.name} — ${s.power || '?'}kW, στα ${s.alongKm} km, άφιξη ~${s.arriveSoc}% → φόρτιση ${s.leaveSoc ?? b.chargeToSoc}%`).join('\n')
        : `  (καμία στάση — άφιξη ~${parsed.arriveSoc}% χωρίς φόρτιση)`;
      const reviewPrompt =
`Είσαι ΑΝΕΞΑΡΤΗΤΟΣ ελεγκτής (δεύτερη γνώμη). Ένα άλλο σύστημα (Claude) πρότεινε το παρακάτω σχέδιο φόρτισης με ΧΙΛΙΟΜΕΤΡΙΚΟ κριτήριο (μέγιστη απόσταση ανά φόρτιση, ΟΧΙ τιμή). Έλεγξε αν είναι σωστό.

Διαδρομή: ${t.from} → ${t.to}, ${t.routeKm} km. Αυτονομία ≈ ${b.rangeFull} km.
Εκκίνηση ${b.startSoc}% · εφεδρεία ${b.reserveSoc}% · φόρτιση έως ${b.chargeToSoc}%.

Διαθέσιμοι υποψήφιοι (km = θέση· "άφιξη %" = SoC χωρίς ενδιάμεση φόρτιση):
${lines}

ΠΡΟΤΑΣΗ CLAUDE:
${stopsTxt}

Έλεγξε αυστηρά: (1) καμία στάση δεν αφήνει το SoC να πέσει κάτω από ${b.reserveSoc}%; (2) είναι όντως η ΠΙΟ ΜΑΚΡΙΝΗ ασφαλής επιλογή ανά σκέλος (μέγιστη απόσταση); (3) υπάρχει καλύτερος υποψήφιος ή λάθος;
Ξεκίνα με «✅ Συμφωνώ» ή «⚠️ Προσοχή» και δώσε 1–3 σύντομα σημεία στα ελληνικά. Μέγιστο ~110 λέξεις. Αγνόησε τιμές.`;
      try {
        const rv = await callAI('openai', reviewPrompt, 500);
        review = { text: rv.text, model: rv.model, provider: 'openai' };
      } catch (e) { review = { error: e.message, provider: 'openai' }; }
    }

    res.json({ plan: parsed, raw: parsed ? null : text, model: result.model, provider: planProvider, review });
  } catch (err) {
    res.status(500).json({ error: `Αποτυχία: ${err.message}` });
  }
});
