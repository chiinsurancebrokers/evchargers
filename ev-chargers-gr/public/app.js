...
function savePrefs() {
  try {
    localStorage.setItem('evgr-prefs', JSON.stringify({
      ...
      soc: els.soc.value, target: els.target.value, chargeto: els.chargeto.value,
...
async function requestBestCharge() {
  ...
  const candidates = lastHits.map((h) => ({
    name: h.c.name || 'Σταθμός φόρτισης',
    power: h.c.maxKW,
    alongKm: Math.round(h.along),
    arriveSoc: Math.round(v.startSoc - (h.along / v.rangeFull) * 100),
    detour: +h.dist.toFixed(1),
    status: h.c.availableMaxKW > 0 ? 'διαθέσιμος' : 'άγνωστο/κατειλημμένος',
  }));

  expandSheetIfMobile();
  els.bestBtn.disabled = true;
  out.innerHTML = '<span class="muted">Το Claude υπολογίζει το βέλτιστο σημείο…</span>';
  refreshEmptyState();
  try {
    ...
    const data = await res.json();
    if (!res.ok) { out.innerHTML = `<span class="err">${escapeHtml(data.error || 'Σφάλμα.')}</span>`; refreshEmptyState(); return; }
    renderBestPlan(data, v);
  } catch (e) {
...
function renderBestPlan(data, v) {
  bestLayer.clearLayers();
  const out = els.bestOut;
  const p = data.plan;
  if (!p) {
    out.innerHTML = `<div class="bc-head">🔋 Βέλτιστο σημείο</div><div class="ai-body">${escapeHtml(data.raw || 'Δεν ήταν δυνατός ο υπολογισμός.')}</div>`;
    refreshEmptyState();
    return;
  }
  let html = `<div class="bc-head">🔋 Βέλτιστο σημείο φόρτισης <span class="ne-from">χιλιομετρικό κριτήριο</span></div>`;
  if (p.needsCharge === false || !(p.stops && p.stops.length)) {
    html += `<div class="bc-ok">Δεν χρειάζεται φόρτιση — εκτιμώμενη άφιξη ~${Math.round(p.arriveSoc || 0)}%.</div>`;
  } else {
    html += '<ol class="bc-list">';
    let n = 0;
    for (const s of p.stops) {
      n++;
      const hit = lastHits.reduce((best, h) =>
        Math.abs(h.along - s.alongKm) < Math.abs((best ? best.along : 1e9) - s.alongKm) ? h : best, null);
      if (hit) {
        bestLayer.addLayer(
          L.marker([hit.c.lat, hit.c.lng], {
            icon: L.divIcon({ className: '', html: `<div class="bc-pin">${n}</div>`, iconSize: [28, 28], iconAnchor: [14, 14] }),
            zIndexOffset: 1600,
          }).bindPopup(`<b>${escapeHtml(s.name || '')}</b><br>${s.power || hit.c.maxKW} kW · ${s.alongKm} km`)
        );
      }
      html += `<li><span class="bc-n">${n}</span><span class="bc-body"><b>${escapeHtml(s.name || 'Σταθμός')}</b> · ${s.power || ''} kW` +
        `<br><span class="bc-meta">στα ${s.alongKm} km · άφιξη ~${Math.round(s.arriveSoc || 0)}% → φόρτιση ${s.leaveSoc || v.chargeToSoc}%</span>` +
        (s.reason ? `<br><span class="bc-why">${escapeHtml(s.reason)}</span>` : '') +
        `</span></li>`;
    }
    html += '</ol>';
    if (p.arriveSoc != null) html += `<div class="bc-arrive">Άφιξη στον προορισμό ~${Math.round(p.arriveSoc)}%</div>`;
  }
  if (p.summary) html += `<div class="bc-sum">${escapeHtml(p.summary)}</div>`;
  const proposer = data.provider === 'openai' ? 'OpenAI' : 'Claude';
  html += `<div class="ai-foot">πρόταση: ${proposer}${data.model ? ` (${escapeHtml(data.model)})` : ''}</div>`;
  // Έλεγχος OpenAI (δεύτερη γνώμη πάνω στην πρόταση του Claude)
  if (data.review) {
    if (data.review.error) {
      html += `<div class="bc-review"><div class="op-head">🟢 Έλεγχος OpenAI</div><div class="err">${escapeHtml(data.review.error)}</div></div>`;
    } else {
      html += `<div class="bc-review"><div class="op-head">🟢 Έλεγχος OpenAI <span class="op-model">${escapeHtml(data.review.model || '')}</span></div>` +
        `<div class="ai-body">${escapeHtml(data.review.text || '').replace(/\n/g, '<br>')}</div></div>`;
    }
  }
  out.innerHTML = html;
  refreshEmptyState();
