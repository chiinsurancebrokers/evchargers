...
function renderPlan(line, hits) {
  ...
  html += '</ol>';
  html += `<button id="ai-btn" class="ghost ai">🤖 Δεύτερη γνώμη με Claude</button>`;
  html += `<div id="ai-out" class="ai-out hidden"></div>`;
...
function refreshEmptyState() {
  ...
  els.resultsEmpty.style.display = (hasNearest || hasPois || hasPlan || hasList) ? 'none' : '';
}
...
els.plan.addEventListener('click', (e) => {
  if (e.target && e.target.id === 'ai-btn') requestSecondOpinion();
});
